...
class LocalObatRepository(
    ...
    private val dao = database.obatDao()

    fun getAll(): Flow<List<ObatEntity>> = dao.getAll()

    fun getById(id: String): Flow<ObatEntity?> = dao.getById(id)

    fun upsert(entity: ObatEntity) {
        scope.launch {
